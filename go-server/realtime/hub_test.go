...
func TestHubMatchReconnections(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Opponent disconnect: preserves memory state for reconnection",
			test: func(t *testing.T) {
				store := &mockDataStore{}
				hub := NewHub(store)
		...
		{
			name: "Player reconnect: sends active_match to reconnected player",
			test: func(t *testing.T) {
				store := &mockDataStore{}
				hub := NewHub(store)
		...
		{
			name: "Match finish: cleans up active match from memory",
			test: func(t *testing.T) {
				store := &mockDataStore{}
				hub := NewHub(store)
